...
class DatasetFromFolder(data.Dataset):   #训练的数据集的格式
    def __init__(self, HR_dir, LR_dir, patch_size, upscale_factor, data_augmentation, transform=None):
        super(DatasetFromFolder, self).__init__()
        self.hr_image_filenames = [join(HR_dir, x) for x in listdir(HR_dir) if is_image_file(x)]  #加载数据位置
...
class DatasetFromFolderEval(data.Dataset):   #测试的数据集的格式
    def __init__(self, HR_dir, LR_dir, upscale_factor, transform=None):
        super(DatasetFromFolderEval, self).__init__()
        self.hr_image_filenames = [join(HR_dir, x) for x in listdir(HR_dir) if is_image_file(x)]  #标签数据集加载
        self.lr_image_filenames = [join(LR_dir, x) for x in listdir(LR_dir) if is_image_file(x)]  #数据集加载
        self.upscale_factor = upscale_factor
        self.transform = transform
